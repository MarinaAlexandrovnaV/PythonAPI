...
class Logger:
    ...
    # работая с classmethod появляется возможность обращаться к переменным класса через ключевое слово cls. Обращаться к переменной хранящей в себе название файла куда нужно будет писать лог
    @classmethod
    ...
    def add_request(cls, url: str, data: dict, headers: dict, cookies: dict, method: str):
        # информация о том, какой сейчас запущен тест и как он называется
